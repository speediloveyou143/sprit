...
const UpdateCourse = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/courses/get-course/${id}`);
        setFormData(response.data);
      } catch (error) {
        alert(error.response?.data?.error || 'Failed to fetch course');
      }
    };
    fetchCourse();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/courses/update-course/${id}`, formData);
